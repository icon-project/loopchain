...
"""block verifier for version 0.4 block"""

from typing import TYPE_CHECKING, Callable, Sequence

from legacy import configure as conf
from legacy.blockchain.blocks import BlockVerifier as BaseBlockVerifier
from legacy.blockchain.blocks.v0_4 import BlockHeader, BlockBody, BlockBuilder
from legacy.blockchain.exception import NotInReps
from legacy.blockchain.types import ExternalAddress, Hash32
from legacy.blockchain.votes.v0_4 import BlockVotes, LeaderVotes
...
class BlockVerifier(BaseBlockVerifier):
    ...
    def verify_invoke(self, builder: 'BlockBuilder', block: 'Block', prev_block: 'Block') -> dict:
        ...
        if header.next_reps_hash != new_header.next_reps_hash:
            if new_header.prep_changed or header.next_reps_hash != new_header.revealed_next_reps_hash:
                exception = RuntimeError(f"Block({header.height}, {header.hash.hex()}, "
