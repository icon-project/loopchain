...
from legacy.blockchain.types import Hash32, ExternalAddress
from legacy.blockchain.votes import Votes as BaseVotes
from legacy.blockchain.votes.v0_5 import BlockVote, LeaderVote
...
class BlockVotes(BaseVotes[BlockVote]):
    ...
    def verify_vote(self, vote: BlockVote):
        ...
        if vote.round != self.round:
            raise RuntimeError(f"Vote round not match. {vote.round} != {self.round}\n"
                               f"{vote}")
...
class LeaderVotes(BaseVotes[LeaderVote]):
    ...
    def deserialize(cls, votes_data: List[Dict], voting_ratio: float):
        if votes_data:
            ...
            votes_instance = cls(reps, voting_ratio, votes[0].block_height, votes[0].round, votes[0].old_leader)
            for vote in votes:
