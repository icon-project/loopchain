...
class PeerLoader:
    ...
    @staticmethod
    async def load(peer_manager: 'PeerManager', channel_name, node_type: conf.NodeType):
        PeerLoader.load_peers_from_iiss(peer_manager, channel_name=channel_name)

        if not peer_manager.peer_list:
            if conf.NodeType.is_support_node_function(conf.NodeFunction.Vote, node_type=node_type):
                await PeerLoader._load_peers_from_file(peer_manager, channel_name=channel_name)
            else:
                await PeerLoader._load_peers_from_rest_call(peer_manager, channel_name)

        utils.logger.debug(f"peer_service:show_peers ({channel_name}): ")
    ...
    @staticmethod
    def load_peers_from_iiss(peer_manager: 'PeerManager', channel_name):
        request = {
    ...
    @staticmethod
    async def _load_peers_from_file(peer_manager: 'PeerManager', channel_name):
        utils.logger.debug(f"load_peers_from_file")
        channel_info = utils.load_json_data(conf.CHANNEL_MANAGE_DATA_PATH)
        reps: list = channel_info[channel_name].get("peers")
        for peer in reps:
            peer_manager.add_peer(peer)

    @staticmethod
    async def _load_peers_from_rest_call(peer_manager: 'PeerManager', channel_name):
        ...
        reps: list = response['channel_infos'][channel_name].get('peers')
