...
class BlockSync:
    ...
    def __init__(self, block_manager: 'BlockManager', channel_service: 'ChannelService'):
        ...
        self._sync_finish_event: Optional[asyncio.Event] = None
    ...
    def _block_request_to_peers_in_sync(
        ...
        async def synchronizer():
            ...
            self._sync_finish_event = asyncio.Event(loop=asyncio.get_event_loop())
    ...
    async def _block_request(self, peer_stubs: List[Tuple], block_height: int):
        ...
        while self._max_height > block_height:
            ...
            self._sync_peer_target[request_height], peer_stub = peer_stubs[self._peer_index]
            try:
            ...
            if response_code == message_code.Response.success:
                if (len(peer_stubs) > 1 and
                    ...
                    self._peer_index = (self._peer_index + 1) % len(peer_stubs)
            ...
            if self._max_height <= block_height and len(self._sync_request_result) > 0:
                utils.logger.debug(f"wait sync finish: max_height = {self._max_height}")
                await self._sync_finish_event.wait()
                utils.logger.debug(f"sync finish event released. max_height: {self._max_height}")
                if self._max_height <= block_height:
                    break

                utils.logger.debug(f"max_height({self._max_height}) "
    ...
    def _update_max_height(self, peer_stubs, sync_height: int) -> bool:
        ...
        """
        result = False
        if self._max_height <= sync_height and not self._sync_finish_event.is_set():
            utils.logger.debug(f"check new max height")
            try:
                _, peer_stub = peer_stubs[self._peer_index]
                ...
                utils.logger.debug(f"sync finish event set()")
                self._sync_finish_event.set()
