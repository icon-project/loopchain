...
class BlockChain:
    ...
    def find_block_info_by_hash(self, hash) -> bytearray:
        hash_encoded = hash.hex().encode(encoding='UTF-8')

        try:
            return self.__confirmed_block_db.Get(BlockChain.BLOCK_INFO_KEY + hash_encoded)
        except KeyError:
            return bytearray(b"")

    def find_block_info_by_height(self, height) -> bytearray:
        block = self.find_block_by_height(height)
        if block:
            return self.find_block_info_by_hash(block.header.hash)
