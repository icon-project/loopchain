...
from loopchain.blockchain.transactions import TransactionBuilder, TransactionVerifier, TransactionVersioner
...
class TestTxCache:
    ...
    def test_no_cache_attr_in_tx_if_not_verified(self, tx_version, tx_factory, target_attr):
        """Check that Transaction has no attribute until the verification func has been invoked"""
        tx = tx_factory(tx_version=tx_version)
        if "cache" in dir(tx):
    ...
    def test_has_cache_attr_when_verified_successfully(self, tx_version, tx_factory, raise_exception, target_attr):
    ...
    def test_exception_cached_when_raised_exception_while_verification(self, tx_version, tx_factory, raise_exception, expected_exc, target_attr, fake_value, monkeypatch):
    ...
    def test_verify_success_and_no_exc_with_fake_value_at_second(self, tx_version, tx_factory, raise_exception, expected_exc, target_attr, fake_value, monkeypatch):
    ...
    @pytest.mark.parametrize("target_attr", ["hash", "signature"])
    def test_benchmark_verify(self, benchmark, tx_version, tx_factory, target_attr, tag):
